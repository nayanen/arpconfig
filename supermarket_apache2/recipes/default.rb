...
if node['apache']['version'] == '2.4' && !platform_family?('freebsd')
  ...
  if node['apache']['mpm_support'].include?(node['apache']['mpm'])
    include_recipe "supermarket_apache2::mpm_#{node['apache']['mpm']}"
  else
    Chef::Log.warn("supermarket_apache2: #{node['apache']['mpm']} module is not supported and must be handled separately!")
  end
end

node['apache']['default_modules'].each do |mod|
  module_recipe_name = mod =~ /^mod_/ ? mod : "mod_#{mod}"
  include_recipe "supermarket_apache2::#{module_recipe_name}"
end
